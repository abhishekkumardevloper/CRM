...
// When the sales agent views this page, clear the "NEW" badge from their main leads list.
...
<?php if (isset($_GET['status']) && $_GET['status'] === 'just_completed'): ?>
    <p class="status-message" style="background-color: #d4edda; color: #155724; padding: 10px; border-radius: 5px; margin-bottom: 20px;">Visit marked as completed! You can now follow up with the client.</p>
<?php endif; ?>

<h2>Site Visit Done Leads</h2>
<p>These visits have been completed. Click "Update" to add final notes, review history, and convert to a customer.</p>
    ...
    <tbody>
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                // --- START: SIMPLIFIED ACTION COLUMN ---
                echo "<td data-label='Action'>
                        <a href='update_lead.php?lead_id=" . $row['lead_id'] . "' class='btn-action btn-manage' style='width: 100%; box-sizing: border-box;'>
                            Update
                        </a>
                      </td>";
                // --- END: SIMPLIFICATION ---
